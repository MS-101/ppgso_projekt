...
private:
    ...
    glm::vec3 bezierPoint(const glm::vec3 controlPoints[4], float t) {
        ...
        point.z = (1-t)*(1-t)*(1-t)*p0.z + 3*(1-t)*(1-t)*t*p1.z + 3*(1-t)*t*t*p2.z + t*t*t*p3.z;
    ...
    BezierPatch(const glm::vec3 controlPoints[4][4]) {
        ...
        for(unsigned int i = 0; i < PATCH_SIZE ; i++) {
            for (unsigned int j = 0; j < PATCH_SIZE; j++) {
                ...
                int k;
                glm::vec3 my_controlPoints[4];
                for (k = 0; k < 4; k++) {
                    my_controlPoints[k] = bezierPoint(controlPoints[k], u);
                }
                glm::vec3 this_bezierPoint = bezierPoint(my_controlPoints, v);
                vertices.push_back(this_bezierPoint);

                glm::vec2 this_texCoords = {u, v};
                texCoords.push_back(this_texCoords);
            }
        }
        // Generate indices
        for(unsigned int i = 1; i < PATCH_SIZE; i++) {
            for (unsigned int j = 1; j < PATCH_SIZE; j++) {
                ...
                unsigned int bottomLeft_id = (i-1)*PATCH_SIZE + j-1;
                unsigned int bottomRight_id = (i-1)*PATCH_SIZE + j;
                unsigned int topLeft_id = i*PATCH_SIZE + j-1;
                unsigned int topRight_id = i*PATCH_SIZE + j;

                mesh.push_back({bottomLeft_id, bottomRight_id, topRight_id});
                mesh.push_back({bottomLeft_id, topLeft_id, topRight_id});
            }
        }
...
public:
    ...
    void onIdle() final {
        ...
        for (int k=0; k<4;k++){
            for (int l=0; l<4;l++){
                vlna[k][l] = controlPoints[k][l];
                vlna[k][l].z = vlna[k][l].z + float(sin(time*float(l/2.0)));
            }
        }
        BezierPatch bezier = BezierPatch(vlna);
